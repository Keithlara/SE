...
ensureAppSchema();
$token = 'QAARCH' . time() . random_int(100,999);
$userEmail = strtolower($token) . '@example.com';
$roomName = 'Room ' . $token;
$orderId = 'ORD-' . $token;
$ticketCode = 'TIC-' . substr(preg_replace('/[^A-Z0-9]/', '', strtoupper($token)), 0, 16);
$bookingId = 900000 + random_int(1000,9999);
mysqli_begin_transaction($con);
try {
  mysqli_query($con, "INSERT INTO `user_cred` (`name`,`email`,`username`,`address`,`phonenum`,`pincode`,`dob`,`password`,`is_verified`,`verification_code`,`status`,`profile`,`is_archived`) VALUES ('{$token}','{$userEmail}','{$token}','QA Address','09171234567',1234,'2000-01-01','test',1,NULL,1,'default.png',0)");
  ...
  mysqli_query($con, "INSERT INTO `booking_order` (`booking_id`,`user_id`,`room_id`,`check_in`,`check_out`,`arrival`,`refund`,`booking_status`,`order_id`,`trans_id`,`trans_amt`,`trans_status`,`trans_resp_msg`,`rate_review`,`datentime`,`payment_status`,`payment_proof`,`refund_proof`,`refund_amount`,`amount_paid`,`confirmed_at`,`total_amt`,`downpayment`,`balance_due`,`promo_code`,`discount_amount`,`is_archived`) VALUES ({$bookingId},{$userId},{$roomId},'2026-05-01','2026-05-03',0,0,'cancelled','{$orderId}','TX-{$token}',800,'TXN_SUCCESS','ok',0,NOW(),'paid','uploads/billing_proofs/{$token}.pdf',NULL,0,800,NOW(),1600,800,800,'QA10',100,1)");
  ...
  mysqli_query($con, "INSERT INTO `rating_review` (`booking_id`,`room_id`,`user_id`,`rating`,`review`,`seen`,`datentime`) VALUES ({$bookingId},{$roomId},{$userId},5,'QA review',0,NOW())");
  $reviewSrNo = (int)mysqli_insert_id($con);
  ...
  mysqli_query($con, "DELETE FROM `rating_review` WHERE `sr_no`={$reviewSrNo}");
  ...
  $restoreChecks = [
    ...
    'rating_review' => "SELECT COUNT(*) c FROM `rating_review` WHERE `sr_no`={$reviewSrNo}",
  ];
